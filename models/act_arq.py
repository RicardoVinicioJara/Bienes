...
class Arqueologia(models.Model):
    ...
    marfologia_id = fields.Char(string='Morfología', required = False, help='Morfología')
...
class Marfologia(models.Model):
    ...
    def name_get(self):
        result = []
        for record in self:
            result.append((record.id, str(record.marfologia)))
        return result
...
class TecnicasElaboracion(models.Model):
    ...
    def name_get(self):
        result = []
        for record in self:
            result.append((record.id, str(record.tecnicas_elaboracion)))
        return result
